...
const BASE_URL = environment.signingBaseUrl;
...
type SignerRecord = Record<string, unknown> & {
  role?: string;
  status?: string;
...
export const expireRequests = async (
  ...
  try {
    ...
    for (const doc of snap.docs) {
      const data = doc.data();
      const signers: SignerRecord[] = data.signers ?? [];

      // Check if any pending signer token is expired
      const hasExpiredToken = signers.some((s) => {
        if (s.role !== "needsToSign" || s.status !== "pending") {
          return false;
        }
        return (
          s.tokenExpiry instanceof Timestamp && s.tokenExpiry.toDate() < now
        );
      });
...
export const resendGuestSigningLink = async (
  ...
  try {
    ...
    const signers: SignerRecord[] = requestData.signers || [];
    ...
    const signerAtIndex = signers[signerIndex];
    signerAtIndex.signingToken = newToken;
    signerAtIndex.tokenExpiry = Timestamp.fromDate(expiresAt);
    ...
    const updatePayload: Record<string, unknown> = { signers };
